...
int main()
{
    ...
    //
    /* To run the program in terminal write gcc (file name)
     a.exe file will be created in the folder
     in terminal wirte a and then press tab


     c fille in vs code --> Compiler (gcc)--> file.exe

    A compiler is a computer program which converts a c program in
    machine lang so that it can be eaily understood by the computer

    C program is written in plain text. The compiler perform checks
    and converts into an executable

    Library functions

    C lang has lot of library fuction which is used to carry out certain task
    printf to print value on screen
    ...
    */
    ...
    // ? Format specifierv for printing adress is (%u)
    /* The value at adress operator (*)
    the value at address or operator is used to obtain the value
    present atv a given memory address it is denoted by (*) */
    ...
    // !Arrays: It is simalar item collection
    int marks[4];
    marks[0] = 35;
    marks[1] = 32;
    marks[2] = 34;
    ...
    // The memory will store the value in form of bytesif arcetecture of system is 4 byte and 3 values are stored then
    // 12 bytes  4 byte for each intiger
    ...
    // !Strings : 1D array termintaed by a null (\0)
    char str[] = {'h', 'b', 'c', '\0'};
    ...
    // !File I/o
    /*Ram read the file before it is shoen to the public
    Ram is volatile and its content is loss once programm is terminated
    in order to persist the data foreve we use file*/

    /*
    A file is data stored in a storage device .A c program can talk to the file
    by reading conten from it and writing content to it.
     */

    // * File Pointer : A file is a structure which need to be created foropening a file

    FILE *ptr;
    ptr = fopen("filename.exe", "mode");

    // !Dynamic Memory Allocation
    /*C is a language with fixed rules of programming
    change the size of an array is not allowed

    Dynamic Memory Allocation
    Dynamic memory allocation is a way to allocate memory to a data structure
    during runtime we can us DMA function available in C to allocate and
    free memory during ruun time


    */
    // ! Malloc()function: It takes number of bytes to be allocate as an
